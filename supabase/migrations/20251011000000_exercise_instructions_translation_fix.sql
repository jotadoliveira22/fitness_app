-- Corrige la migración anterior (20251010000000): esa migración solo
-- pisaba exercises.instructions donde estaba en null, pero la importación
-- SUMIVA (20251003000000_sumiva_import.sql) ya había cargado ahí el texto
-- original en inglés de Free Exercise DB (columna instrucciones_original),
-- así que ninguna fila calificaba y la traducción nunca se aplicó.
--
-- Esta migración pisa exercises.instructions para los mismos 866 ejercicios,
-- sin la condición "is null", matcheando por nombre exacto (mismo dataset
-- que la migración anterior). Es seguro: son los mismos nombres en español
-- ya validados, no toca ejercicios de otras fuentes (seeds manuales).
...
-- Verificación: cuántos ejercicios todavía tienen texto en inglés
-- reconocible (heurística simple: contienen alguna palabra común en inglés
-- de instrucciones que no aparece en español).
